...
let nameAsText = JSON.stringify(posts[0].benutzername);
//
localStorage.setItem(`posts`, commentsAsText);
localStorage.setItem(`posts`, nameAsText);
}


function load(){
let commentsAsText = localStorage.getItem(`posts`);
let nameAsText = localStorage.getItem(`benutzername`);
//
if (commentsAsText) {posts[0].comments=JSON.parse(commentsAsText);}
if (nameAsText){posts[0].benutzername=JSON.parse(nameAsText);}
}
*/
